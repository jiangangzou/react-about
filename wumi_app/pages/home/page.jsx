...
export default class extends Component {
  ...
  loadTypeData ({ latitude, longitude}) {
    fetch('https://www.easy-mock.com/mock/5b62caee061b7876217a9e0c/abc/#!method=get')
      .then(res => {
        if (res.status === 200) {
          res.json().then(data => {
            this.setState({
  ...
  loadPoiData ({ latitude, longitude}) {
    ...
    fetch(`https://www.easy-mock.com/mock/5b62caee061b7876217a9e0c/abc/#!method=get`)
      .then(res => {
        if (res.status === 200) {
          res.json().then(data => {
            // console.log(data[1].scenes.name)
            this.setState({
              address: data[1].scenes.name
            })
          })
        }
      })
  ...
  loadRestaurantData ({ latitude, longitude},rank_id='') {
    ...
    //       this.dataSource = this.dataSource.cloneWidthRows(this.rests);
    //       this.setState({
